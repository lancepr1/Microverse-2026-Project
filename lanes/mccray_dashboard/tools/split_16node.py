...
named "<hostname>.jsonl", to data/ (see data/README.md for the full
writeup).
...
def split(input_path: Path, output_dir: Path) -> dict[str, int]:
    ...
    counts = {}
    for hostname, records in sorted(per_node_records.items()):
        out_path = output_dir / f"{hostname}.jsonl"
        with open(out_path, "w") as f:
            for record in records:
                f.write(json.dumps(record) + "\n")
        counts[hostname] = len(records)
...
def main():
    ...
    parser.add_argument("--output-dir", type=Path, default=_DEFAULT_OUTPUT_DIR,
                         help="Directory to write <hostname>.jsonl files into")
    args = parser.parse_args()

    counts = split(args.input, args.output_dir)
    for hostname, count in counts.items():
        print(f"{hostname}: {count} rows -> {args.output_dir / (hostname + '.jsonl')}")
